...
function minersNeeded(room: Room, capacity: number, spaces: number): number {
  ...
  return Math.min(spaces, Math.ceil(capacity / (ENERGY_REGEN_TIME * HARVEST_POWER * work)));
}
...
function runTowers(room: Room) {
  ...
  for (const tower of towers) {
    ...
    const enemy = tower.pos.findClosestByRange(hostiles);
    if (enemy) { tower.attack(enemy); continue; }                              // 2: defend
...
export function loop() {
  ...
  for (const name in Game.spawns) {
    ...
    if (econ instanceof Goal.BuildSettler && !settlerExists(econ.settle)) {
      ...
      else if (tNeed > 0 && (wNeed <= 0 || tRatio <= wRatio)) spawnHauler(s);
...
namespace Unit {
  ...
  function pickResource(room: Room, pos?: RoomPosition, includeEmpty = false): Source | Mineral | undefined {
  ...
  export class Worker extends Obj {
    ...
    private remoteHaul(r: { src: Id<Source>; room: string; home: string }): boolean {
      const creep = this.self;
      const did = this.buildAlong(); // haulers pave the route too
      if (did === 'move') return true; // detouring to a site → let it finish before hauling
      // On the way back (carrying energy), if we didn't build this tick, repair the most-worn road/container
      // we pass — lowest %HP first. build and repair are both work-actions, so only one or the other.
      if (did === 'none' && (creep.store[RESOURCE_ENERGY] ?? 0) > 0) {
        const worn = this.mostWorn(10);
    ...
    // The most worn (lowest %HP) damaged road/container within `range`, for opportunistic repair.
    private mostWorn(range: number): Structure | undefined {
      const worn = this.self.pos.findInRange(FIND_STRUCTURES, range, {
        filter: s => (s.structureType === STRUCTURE_ROAD || s.structureType === STRUCTURE_CONTAINER) && s.hits < s.hitsMax,
      });
      return worn.length ? worn.reduce((a, b) => (a.hits / a.hitsMax <= b.hits / b.hitsMax ? a : b)) : undefined;
    }
    ...
    protected acquire() {
      ...
      const source = sticky && current instanceof Source && availableSpacesAround(current) > 0
        ? current
        : pickResource(creep.room, creep.pos, sticky); // container mode → may target an empty source to wait at it
  ...
  export class Transporter extends Worker {
    loop() {
      if (this.self.memory.dedicated) return this.shuttle(); // dedicated shuttle stays in its own loop
      super.loop();
    }
    ...
    // Cover a miner shortfall — but hauling is the real job, so only stand in as a miner when there is
    // genuinely NOTHING to haul (every container empty), workers are actually short (a spot uncovered),
    // AND there's a free source spot with energy to mine. Re-decided each empty trip (not sticky), so the
    // moment a container has energy — or workers catch up — the transporter goes back to hauling instead of
    // parking at a source. Self-limits via spot occupancy (pickResource stops handing out taken spots).
    private updateStandIn(): void {
      const creep = this.self, room = creep.room;
      const hasHaul = room.find(FIND_STRUCTURES, {
        filter: s => s.structureType === STRUCTURE_CONTAINER && (s as StructureContainer).store[RESOURCE_ENERGY] > 0,
      }).length > 0;
      // Hauling is the real job: the moment ANY container has energy, stop standing in as a miner — even
      // mid-load (otherwise a stand-in whose source ran dry would sit idle holding a partial load while a
      // container needs hauling).
      if (hasHaul) { creep.memory.mining = undefined; return; }
      if (this.carrying !== undefined) return; // nothing to haul + carrying → keep mining toward a full tank
      // Only stand in if miners + transporters ALREADY standing in are still short of the spots — counting
      // the OTHERS (not ourselves) so a crowd of empty transporters doesn't all trek to the same free spot.
      const miners = room.find(FIND_MY_CREEPS, { filter: isLocalWorker }).length;
      const standins = room.find(FIND_MY_CREEPS, { filter: c => c.id !== creep.id && isTransporter(c) && !!c.memory.mining }).length;
      const shortMiners = miners + standins < workerTarget(room);
      creep.memory.mining = (shortMiners && !!pickResource(room, creep.pos)) || undefined;
    }

    // Mine while standing in for a missing miner; otherwise withdraw from a container to haul.
    protected acquire() {
      this.updateStandIn();
      if (this.self.memory.mining) return this.standInMine();
    ...
    // Stand-in mining: just harvest the closest source that has energy and a free spot — NO static-miner
    // "stay parked at an empty source" stickiness (a hauler shouldn't loiter). The mined load drops into a
    // container in deliver() (wantsContainerDrop is true while mining).
    private standInMine(): void {
      const creep = this.self;
      const source = pickResource(creep.room, creep.pos);
      if (!source) { if ((creep.store[RESOURCE_ENERGY] ?? 0) > 0) this.deliver(); return; } // can't mine → at least deliver what we hold, don't idle
      creep.memory.target = source.id as Id<Source>;
      if (creep.pos.isNearTo(source)) creep.harvest(source);
      else creep.moveTo(source, { visualizePathStyle: { stroke: '#ffaa00' } });
    }

    // While standing in as a miner, drop into containers like one; otherwise (a hauler) never put energy
    // back into a container — we just took it out.
    protected wantsContainerDrop(): boolean { return this.self.memory.mining ? super.wantsContainerDrop() : false; }
